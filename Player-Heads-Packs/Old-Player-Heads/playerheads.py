...
customs = []
farm_products = []
person_text = "entity.playerhead:lower.name=template's Head"
person_egg_text = "item.spawn_egg.entity.playerhead:lower.name=template's Head"
microblock_text = "entity.playerhead:lower.name=Micro-template"
microblock_egg_text = "item.spawn_egg.entity.playerhead:lower.name=Micro-template"
...
custom_text = "entity.playerhead:lower.name=template"
custom_egg_text = "item.spawn_egg.entity.playerhead:lower.name=template"
...
if __name__ == '__main__':
    ...
    for head in heads:
        ...
        with open('in_progress/resource_pack/texts/en_US.lang', 'a') as outfile:
            if head in people:
                copy('skins/people/' + head + '.png','in_progress/resource_pack/textures/entity/playerheads/' + head + '.png')
                outfile.write(person_text.replace('template', head.title().replace('_',' ')).replace('lower', head.lower()).replace('Of ', 'of').replace('And ', 'and') + '\n')
                outfile.write(person_egg_text.replace('template', head.title().replace('_',' ')).replace('lower', head.lower()).replace('Of ', 'of').replace('And ', 'and') + '\n\n')
            if head in microblocks:
                #TNT is an exception because it is all caps
                if head == 'tnt':
                    copy('skins/microblocks/tnt.png','in_progress/resource_pack/textures/entity/playerheads/tnt.png')
                    outfile.write(microblock_text.replace('lower','tnt').replace('template', 'TNT') + '\n')
                    outfile.write(microblock_egg_text.replace('lower','tnt').replace('template', 'TNT') + '\n\n')
                #Jack o'Lantern is an exception because it has a capital in the middle of a word
                if head == 'jack_olantern':
                    copy('skins/microblocks/jack_olantern.png','in_progress/resource_pack/textures/entity/playerheads/jack_olantern.png')
                    outfile.write(microblock_text.replace('lower','jack_olantern').replace('template', "Jack o'Lantern") + '\n')
                    outfile.write(microblock_egg_text.replace('lower','jack_olantern').replace('template', "Jack o'Lantern") + '\n\n')
                else:
                    copy('skins/microblocks/' + head + '.png','in_progress/resource_pack/textures/entity/playerheads/' + head + '.png')
                    ...
                    outfile.write(microblock_egg_text.replace('template', head.title().replace('_',' ')).replace('lower', head.lower()).replace('Of', 'of').replace('And ', 'and ') + '\n\n')
            if head in mobs:
            ...
            if head in customs:
                copy('skins/custom/' + head + '.png','in_progress/resource_pack/textures/entity/playerheads/' + head + '.png')
                outfile.write(custom_text.replace('template', head.title().replace('_',' ')).replace('lower', head.lower()).replace('Of', 'of').replace('And', 'and') + '\n')
                outfile.write(custom_egg_text.replace('template', head.title().replace('_',' ')).replace('lower', head.lower()).replace('Of', 'of').replace('And', 'and') + '\n\n')
            if head in farm_products:
                copy('skins/farm_products/' + head + '.png','in_progress/resource_pack/textures/entity/playerheads/' + head + '.png')
                outfile.write(custom_text.replace('template', head.title().replace('_',' ')).replace('lower', head.lower()).replace('Of', 'of').replace('And', 'and') + '\n')
                outfile.write(custom_egg_text.replace('template', head.title().replace('_',' ')).replace('lower', head.lower()).replace('Of', 'of').replace('And', 'and') + '\n\n')
    ...
    rename('in_progress','Player Heads Addon')
    chdir('Player Heads Addon')
    rename('behavior_pack','playerheads_b')
    '''with ZipFile('playerheads_b.mcpack', 'w') as archive:
        for subdir, dirs, files in walk('playerheads_b'):
            for file in files:
                filepath = join(subdir, file)
                archive.write(filepath)'''
    rename('resource_pack','playerheads_r')
    with ZipFile('playerheads_r.mcpack', 'w') as archive:
